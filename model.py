...
class MultiFidelityModel():
    ...
    def __init__(self, num_fidel, total_budget, costs, kernel_name, true_fns,
        ...
        beta=0.1, train_Xs=None, train_Ys=None, device='cpu'):
        ...
        if train_Xs is None:
            self.f_m = SingleTaskGP(torch.tensor([]).to(device), 
                torch.tensor([]).to(device), covar_module=kernels[kernel_name])
        else:
        ...
        if train_Xs is None:
            for _ in range(num_fidel - 1):
                self.f_ls.append(SingleTaskGP(torch.tensor([]).to(device), 
                    torch.tensor([]).to(device), 
                    covar_module=kernels[kernel_name]))
        else:
            # TODO
            raise Exception("Still unimplemented")
            # f_l is modeled by train_X, train_Y. Need to calculate gaussian 
            # process e_l = f_m - f_l and create a model object
    ...
    def explore_lf(self):
        ...
        if l == self.num_fidel-1:
            break
        elif MI("something") < self.beta:
            break
        else:
            actions.append((x, l))
            action_cost += self.costs[l]

        return set(actions), action_cost

    def sf_gp_opt(self):
        candidate, acq_value = optimize_acqf(self.acq_fn, bounds=self.bounds, 
            q=self.num_candidates, num_restarts=self.num_restarts, 
    ...
    def optimize(self):
        while self.B > 0:
            ...
            for x_l, l in L:
                # query the function and update posterior
                self.f_l[l] = self.f_l[l].condition_on_observations(x_l, 
                    self.true_fns[l](x_l))
            # update posterior of f_m 
            self.f_m = self.f_m.condition_on_observations(x, 
                self.true_fns[self.num_fidel-1](x))
            # how to add GPs?
